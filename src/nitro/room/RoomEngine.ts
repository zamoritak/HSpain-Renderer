...
import { BadgeImageReadyEvent, NitroToolbarAnimateIconEvent, RoomBackgroundColorEvent, RoomDragEvent, RoomEngineAreaHideStateEvent, RoomEngineEvent, RoomEngineObjectEvent, RoomObjectEvent, RoomObjectFurnitureActionEvent, RoomObjectMouseEvent, RoomSessionEvent, RoomToObjectOwnAvatarMoveEvent } from '../../events';
import { GetTicker, GetTickerTime, NitroSprite, TextureUtils } from '../../pixi-proxy';
import { NumberBank, RoomAreaSelectionManager, RoomEnterEffect, RoomGeometry, RoomInstance, RoomObjectUpdateMessage, RoomRendererFactory } from '../../room';
...
export class RoomEngine extends NitroManager implements IRoomEngine, IRoomCreator, IRoomEngineServices, IRoomManagerListener, IRoomContentListener, IUpdateReceiver, IDisposable
{
    ...
    private _furnitureVisibilityTimer: number = 0;
    ...
    public updateAreaHide(roomId: number, furniId: number, on: boolean, rootX: number, rootY: number, width: number, length: number, invert: boolean, invisibility: boolean = false, wallItems: boolean = false, blockWalking: boolean = false, alphaHide: boolean = false): boolean
    {
        const roomObject = this.getRoomOwnObject(roomId);

        if(!roomObject || !roomObject.logic) return false;

        const furniture = this.getRoomObject(roomId, furniId, RoomObjectCategory.FLOOR);

        if(furniture && furniture.model)
        {
            furniture.model.setValue(RoomObjectVariable.FURNITURE_AREA_HIDE_ROOT_X, rootX);
            furniture.model.setValue(RoomObjectVariable.FURNITURE_AREA_HIDE_ROOT_Y, rootY);
            furniture.model.setValue(RoomObjectVariable.FURNITURE_AREA_HIDE_WIDTH, width);
            furniture.model.setValue(RoomObjectVariable.FURNITURE_AREA_HIDE_LENGTH, length);
            furniture.model.setValue(RoomObjectVariable.FURNITURE_AREA_HIDE_INVERT, invert ? 1 : 0);
            furniture.model.setValue(RoomObjectVariable.FURNITURE_AREA_HIDE_INVISIBILITY, invisibility ? 1 : 0);
            furniture.model.setValue(RoomObjectVariable.FURNITURE_AREA_HIDE_WALL_ITEMS, wallItems ? 1 : 0);
            furniture.model.setValue(RoomObjectVariable.FURNITURE_AREA_HIDE_BLOCK_WALKING, blockWalking ? 1 : 0);
            furniture.model.setValue(RoomObjectVariable.FURNITURE_AREA_HIDE_HIDE_FURNITURE, alphaHide ? 1 : 0);
            furniture.setState(on ? 1 : 0, 0);
        }

        this.events.dispatchEvent(new RoomEngineAreaHideStateEvent(roomId, furniId, RoomObjectCategory.FLOOR, on, rootX, rootY, width, length, invert, invisibility, wallItems, blockWalking, alphaHide));

        if(on)
        {
            roomObject.logic.processUpdateMessage(new ObjectRoomFloorHoleUpdateMessage(ObjectRoomFloorHoleUpdateMessage.ADD, furniId, rootX, rootY, width, length, invert, blockWalking, alphaHide, wallItems, invisibility));
        }
        else
        {
            roomObject.logic.processUpdateMessage(new ObjectRoomFloorHoleUpdateMessage(ObjectRoomFloorHoleUpdateMessage.REMOVE, furniId, rootX, rootY, width, length, invert, blockWalking, alphaHide, wallItems, invisibility));
        }
