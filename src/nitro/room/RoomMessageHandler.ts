...
export class RoomMessageHandler extends Disposable
{
    ...
    {
        if(!(event instanceof AreaHideMessageEvent) || !event.connection || !this._roomCreator) return;

        const parser = event.getParser();
        const areaData = parser.areaData;

        this._roomCreator.updateAreaHide(this._currentRoomId, areaData.furniId, areaData.on, areaData.rootX, areaData.rootY, areaData.width, areaData.length, areaData.invert, areaData.invisibility, areaData.wallItems, areaData.blockWalking, areaData.alphaHide);
    }
