...
import { IRoomCanvasMouseListener, IRoomGeometry, IRoomObject, IRoomObjectController, IRoomObjectSprite, IRoomObjectSpriteVisualization, IRoomRenderingCanvas, IRoomSpriteCanvasContainer, IRoomSpriteMouseEvent, MouseEventType, RoomObjectSpriteData, RoomObjectSpriteType, RoomObjectVariable, Vector3d } from '../../api';
import { RoomSpriteMouseEvent } from '../../events';
import { Nitro } from '../../nitro/Nitro';
import { RoomLogic } from '../../nitro/room/object/logic/room/RoomLogic';
...
export class RoomSpriteCanvas implements IRoomRenderingCanvas
{
    ...
    private _roomLogic: RoomLogic = null;
    ...
    private renderObject(object: IRoomObject, identifier: string, time: number, update: boolean, updateVisuals: boolean, count: number): number
    {
        ...
        if(!visualization || this.shouldHideObject(object))
        {
        ...
        for(const sprite of visualization.sprites.values())
        {
            ...
            const width = ((sprite.width > 0) ? sprite.width : texture.width);
            const height = ((sprite.height > 0) ? sprite.height : texture.height);

            if(sprite.flipH)
            {
                const checkX = ((x + (-(width + (-(sprite.offsetX))))) + this._screenOffsetX);

                if(!this.isSpriteVisible(checkX, spriteY, width, height)) continue;
            }

            else if(sprite.flipV)
            {
                const checkY = ((y + (-(height + (-(sprite.offsetY))))) + this._screenOffsetY);

                if(!this.isSpriteVisible(spriteX, checkY, width, height)) continue;
            }

            else
            {
                if(!this.isSpriteVisible(spriteX, spriteY, width, height)) continue;
            }
            ...
            if((sprite.spriteType === RoomObjectSpriteType.AVATAR) || (sprite.spriteType === RoomObjectSpriteType.AVATAR_OWN))
            {
                ...
                if(sprite.width > 0)
                {
                    sortableSprite.sprite.libraryAssetName += '_' + Math.floor(sprite.width);
                }
            }
    ...
    private renderSprite(index: number, sprite: SortableSprite): boolean
    {
        ...
        if(extendedSprite.needsUpdate(objectSprite.id, objectSprite.updateCounter) || RoomEnterEffect.isVisualizationOn())
        {
            ...
            if(objectSprite.width > 0 && objectSprite.texture && objectSprite.texture.width > 0)
            {
                scaleX = objectSprite.width / objectSprite.texture.width;
            }

            if(objectSprite.height > 0 && objectSprite.texture && objectSprite.texture.height > 0)
            {
                scaleY = objectSprite.height / objectSprite.texture.height;
            }

            if(objectSprite.flipH) scaleX = -scaleX;
            if(objectSprite.flipV) scaleY = -scaleY;

            if(extendedSprite.scale.x !== scaleX) extendedSprite.scale.x = scaleX;
            if(extendedSprite.scale.y !== scaleY) extendedSprite.scale.y = scaleY;
    ...
    private checkMouseHits(x: number, y: number, type: string, altKey: boolean = false, ctrlKey: boolean = false, shiftKey: boolean = false, buttonDown: boolean = false): boolean
{
    ...
    while(spriteId >= 0)
    {
        ...
        if(extendedSprite && extendedSprite.containsPoint(new Point((x - extendedSprite.x), (y - extendedSprite.y))))
        {
            if(!extendedSprite.skipMouseHandling)
            {
                ...
                {
                    // Click-handling sprite, but no default behavior here
                }
                else
                {
                    ...
                    if(checkedSprites.indexOf(identifier) === -1)
                    {
                        ...
                        // Detect ROLL_OUT
                        if(mouseData && mouseData.spriteTag !== tag)
                        {
                        ...
                        // Detect ROLL_OVER or other event
                        ...
                        const mouseMoved = (x !== this._mouseOldX) || (y !== this._mouseOldY);
                        if((type !== MouseEventType.MOUSE_MOVE) || mouseMoved)
                        {
                            this.bufferMouseEvent(mouseEvent, identifier);
                        }
    ...
    // ▼▼▼ MODIFICACIÓN: Solo ROLL_OUT en MOUSE_MOVE ▼▼▼
    if(type === MouseEventType.MOUSE_MOVE)
    {
        ...
        {
            if(checkedSprites.indexOf(key) === -1)
            {
    ...
    private getRoomLogic(): RoomLogic
    {
        if(this._roomLogic && !this._roomLogic.disposed) return this._roomLogic;

        const objects = this._container.objects;
        for(const object of objects.values())
        {
            if(object.id === -1) // RoomEngine.ROOM_OBJECT_ID
            {
                this._roomLogic = (object as IRoomObjectController).logic as RoomLogic;
                return this._roomLogic;
            }
        }
    ...
    private shouldHideObject(object: IRoomObject): boolean
    {
        if(!object || object.id === -1 || (object.instanceId === -2) || (object.instanceId === -3)) return false;

        const roomLogic = this.getRoomLogic();
        
        if(!roomLogic || !roomLogic._planeParser) return false;

        if((object.model.getValue(RoomObjectVariable.FIGURE) !== undefined) || (object.model.getValue(RoomObjectVariable.PET_TYPE) !== undefined)) return false;

        if(object.model.getValue(RoomObjectVariable.FURNITURE_AREA_HIDE_ROOT_X) !== undefined) return false;

        const floorHoles = roomLogic._planeParser._floorHoles;
        const floorHolesInverted = roomLogic._planeParser._floorHolesInverted;

        if((!floorHoles || (floorHoles.size === 0)) && (!floorHolesInverted || (floorHolesInverted.size === 0))) return false;

        const location = object.getLocation();
        let x = Math.floor(location.x);
        let y = Math.floor(location.y);

        const isWallItem = object.model.getValue(RoomObjectVariable.FURNITURE_IS_WALL_ITEM) === 1;
        const checkTiles: { x: number, y: number }[] = [];

        if (isWallItem)
        {
            const direction = object.getDirection().x;
            const lx = location.x;
            const ly = location.y;

            if (direction === 90) // Left wall (l)
            {
                // Boundary between floor(lx) and ceil(lx)
                checkTiles.push({ x: Math.floor(lx), y: Math.floor(ly) });
                checkTiles.push({ x: Math.ceil(lx), y: Math.floor(ly) });
            }
            else if (direction === 180) // Right wall (r)
            {
                // Boundary between floor(ly) and ceil(ly)
                checkTiles.push({ x: Math.floor(lx), y: Math.floor(ly) });
                checkTiles.push({ x: Math.floor(lx), y: Math.ceil(ly) });
            }
            else
            {
                checkTiles.push({ x: Math.floor(lx), y: Math.floor(ly) });
        ...
        {
            checkTiles.push({ x: Math.floor(location.x), y: Math.floor(location.y) });
        }



        if(floorHolesInverted && (floorHolesInverted.size > 0))
        {
            let hasAlphaHideInverted = false;
            for(const hole of floorHolesInverted.values())
            {
                if(hole.alphaHide)
                {
                    hasAlphaHideInverted = true;
                    break;
                }
            }

            if(hasAlphaHideInverted)
            {
                ...
                if(!inSafeArea) return true;
            }
        }

        if(floorHoles && (floorHoles.size > 0))
        {
            for(const [id, hole] of floorHoles.entries())
            {
