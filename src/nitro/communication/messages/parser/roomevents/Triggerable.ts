...
export class Triggerable
{
    ...
    // --- Nuevo getter matchState ---
    public get matchState(): number
    {
    return this._intParams && this._intParams.length > 0 ? this._intParams[0] : 0;
    }

    // --- Nuevo getter userSource ---
    public get userSource(): number
    {
    return this._intParams && this._intParams.length > 1 ? this._intParams[1] : 0;
    }

    // --- Nuevo getter furniSource ---
    public get furniSource(): number
    {
    return this._intParams && this._intParams.length > 2 ? this._intParams[2] : 0;
    }
}
