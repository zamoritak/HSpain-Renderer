...
{
    public _planeParser: RoomPlaneParser;
    ...
    public update(time: number): void
    {
        ...
        {
            console.log('[RoomLogic] Processing map update in update() method');
            const model = this.object && this.object.model;

            if(model)
            {
                const mapData = this._planeParser.getMapData();
                console.log('[RoomLogic] Got mapData, holeMap length:', mapData.holeMap?.length || 0);
                ...
                console.log('[RoomLogic] Calling initializeFromMapData to refresh room');
                this._planeParser.initializeFromMapData(mapData);
            }

            this._lastHoleUpdate = 0;
            this._needsMapUpdate = false;
            console.log('[RoomLogic] Map update complete, _needsMapUpdate set to false');
        }
    ...
    {
        return this._planeParser.isFurnitureHidden(x, y, width, height);
    }

    public isWallItemHidden(x: number, y: number): boolean
    {
        return this._planeParser.isWallItemHidden(x, y);
    }
    ...
    public processUpdateMessage(message: RoomObjectUpdateMessage): void
    {
        ...
        {
            this.onObjectRoomFloorHoleUpdateMessage(message);
    ...
    private onObjectRoomFloorHoleUpdateMessage(message: ObjectRoomFloorHoleUpdateMessage): void
    {
        console.log('[RoomLogic] onObjectRoomFloorHoleUpdateMessage - type:', message.type, 'id:', message.id, 'alphaHide:', message.alphaHide, 'invisibility:', message.invisibility);
        
        if(message.type === ObjectRoomFloorHoleUpdateMessage.ADD)
        {
            console.log('[RoomLogic] ADD floor hole - id:', message.id, 'pos:', message.x, message.y, 'size:', message.width, message.height, 'invisibility:', message.invisibility);
            this._planeParser.addFloorHole(message.id, message.x, message.y, message.width, message.height, message.invert, message.blockWalking, message.alphaHide, message.hideWallItems, message.invisibility);
        }
        else
        {
            console.log('[RoomLogic] REMOVE floor hole - id:', message.id);
            this._planeParser.removeFloorHole(message.id, message.x, message.y, message.width, message.height);
        }

        this._needsMapUpdate = true;
        this._lastHoleUpdate = this.time;
        console.log('[RoomLogic] _needsMapUpdate set to true, _lastHoleUpdate:', this._lastHoleUpdate);
    }
