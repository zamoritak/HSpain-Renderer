...
export class RoomPlaneParser
{
    ...
    public _floorHoles: Map<number, RoomFloorHole>;
    public _floorHolesInverted: Map<number, RoomFloorHole>;
    private _floorHoleMatrix: boolean[][];
    private _blockedMatrix: boolean[][];
    ...
    public initializeTileMap(width: number, height: number): boolean
    {
        ...
        while(y < height)
        {
            ...
            this._blockedMatrix.push(blockedMatrix);
    ...
    public getTileHeight(k: number, _arg_2: number): number
    {
        ...
        if(this._blockedMatrix && this._blockedMatrix[_arg_2] && this._blockedMatrix[_arg_2][k])
        {
            return RoomPlaneParser.TILE_BLOCKED;
        }
    ...
    {
        console.log('[RoomPlaneParser] initializeFromTileData START');
        let _local_2: number;
        let _local_3: number;
        ...
        while(_local_3 < this._height)
        {
            ...
            while(_local_2 < this._width)
            {
                ...
                const currentHeight = this._tileMatrix[_local_3][_local_2];
                if(currentHeight > RoomPlaneParser.TILE_HOLE || this._tileMatrixOriginal[_local_3][_local_2] === undefined)
                {
                    this._tileMatrixOriginal[_local_3][_local_2] = currentHeight;
                }
                _local_2++;
            }
            _local_3++;
        ...
        console.log('[RoomPlaneParser] initializeFromTileData - Applying holes temporarily for initialization');
        
        const tempBackup: { x: number, y: number, height: number }[] = [];
        if(this._floorHoleMatrix && this._floorHoleMatrix.length > 0)
        {
            for(let y = 0; y < this._height; y++)
            {
                const row = this._floorHoleMatrix[y];
                if(!row) continue;
                for(let x = 0; x < this._width; x++)
                {
                    if(row[x])
                    {
                        tempBackup.push({ x, y, height: this._tileMatrix[y][x] });
                        this._tileMatrix[y][x] = RoomPlaneParser.TILE_HOLE;
                    }
                }
            }
        }

        const result = this.initialize(_local_4);

        for(const entry of tempBackup)
        {
            this._tileMatrix[entry.y][entry.x] = entry.height;
        }
    ...
    public initializeFromMapData(data: RoomMapData): boolean
    {
        ...
        console.log('[RoomPlaneParser] initializeFromMapData called, holeMap length:', data.holeMap?.length || 0);

        this.reset();

        this.resetFloorHoles();
        console.log('[RoomPlaneParser] Floor holes reset');
        ...
        if(data.holeMap && data.holeMap.length)
        {
            ...
            while(index < data.holeMap.length)
            {
                ...
                if(hole.on)
                {
                    const invert = !!hole.invert;
                    const blockWalking = !!hole.blockWalking;
                    const alphaHide = !!hole.alphaHide;
                    const hideWallItems = !!hole.hideWallItems;
                    const invisibility = hole.invisibility !== undefined ? !!hole.invisibility : true;
                    
                    this.addFloorHole(hole.id, hole.x, hole.y, hole.width, hole.height, invert, blockWalking, alphaHide, hideWallItems, invisibility);
                }
    ...
    public getMapData(): RoomMapData
    {
        ...
        for(const [ holeId, holeData ] of this._floorHoles.entries())
        {
            ...
            data.holeMap.push({
                ...
                on: true,
                invert: false,
                blockWalking: holeData.blockWalking,
                alphaHide: holeData.alphaHide,
                hideWallItems: holeData.hideWallItems,
                invisibility: holeData.invisibility
            });
        }
    ...
    public addFloorHole(k: number, _arg_2: number, _arg_3: number, _arg_4: number, _arg_5: number, _arg_6: boolean = false, blockWalking: boolean = false, alphaHide: boolean = false, hideWallItems: boolean = false, invisibility: boolean = true): void
    {
        invisibility = true;
        
        console.log('[RoomPlaneParser] addFloorHole DEBUG - id:', k, 'alphaHide:', alphaHide, 'invis:', invisibility, 'invert:', _arg_6);
        this.removeFloorHole(k);

        if(_arg_6)
        {
            this._floorHolesInverted.set(k, new RoomFloorHole(_arg_2, _arg_3, _arg_4, _arg_5, blockWalking, alphaHide, hideWallItems, invisibility));
            console.log('[RoomPlaneParser] Added to _floorHolesInverted, new size:', this._floorHolesInverted.size);
        }
        else
        {
            this._floorHoles.set(k, new RoomFloorHole(_arg_2, _arg_3, _arg_4, _arg_5, blockWalking, alphaHide, hideWallItems, invisibility));
            console.log('[RoomPlaneParser] Added to _floorHoles, new size:', this._floorHoles.size);
        }
    }

    public removeFloorHole(k: number, x: number = 0, y: number = 0, width: number = 0, height: number = 0): void
    {
    ...
    public resetFloorHoles(): void
    {
        ...
        {
            this._blockedMatrix[i] = [];
            let j = 0;
            while(j < this._width)
            {
                this._blockedMatrix[i][j] = false;
                j++;
            }
            i++;
        }
    ...
    private getTileHeightOriginal(k: number, _arg_2: number): number
    {
        ...
        if(this._blockedMatrix[_arg_2] && this._blockedMatrix[_arg_2][k])
        {
            return RoomPlaneParser.TILE_BLOCKED;
        }
    ...
    {
        console.log('[RoomPlaneParser] initializeHoleMap START - _floorHoles:', this._floorHoles.size, '_floorHolesInverted:', this._floorHolesInverted.size);
        
        let k: number;
        let _local_2: number;
        let _local_3: boolean[];
        let _local_20: boolean;
        
        _local_2 = 0;
        
        let blockedByInverted = false;
        let invisibilityByInverted = false;
        for(const hole of this._floorHolesInverted.values())
        {
            if(hole.blockWalking) blockedByInverted = true;
            if(hole.invisibility) invisibilityByInverted = true;
            
            if(blockedByInverted && invisibilityByInverted) break;
        }
        
        console.log('[RoomPlaneParser] Clearing _floorHoleMatrix, dimensions:', this._width, 'x', this._height);
        while(_local_2 < this._height)
        {
            _local_3 = this._floorHoleMatrix[_local_2];
            const blockedRow = this._blockedMatrix[_local_2];
            k = 0;
            while(k < this._width)
            {
                _local_3[k] = invisibilityByInverted;
                blockedRow[k] = blockedByInverted; // chorizo
                k++;
            }
            _local_2++;
        }
        console.log('[RoomPlaneParser] Processing', this._floorHolesInverted.size, 'inverted holes');
        for(const _local_4 of this._floorHolesInverted.values())
        {
            this.initializeHole(_local_4, true);
        }
        console.log('[RoomPlaneParser] Processing', this._floorHoles.size, 'normal holes');
        for(const _local_4 of this._floorHoles.values())
        {
            this.initializeHole(_local_4);
        }
        console.log('[RoomPlaneParser] initializeHoleMap COMPLETE');
    }
    ...
    private initializeHole(param1: RoomFloorHole, param2: boolean = false): void
    {
        ...
        if(_local_5 != null)
        {
            ...
            while(_local_2 <= _local_9)
            {
                ...
                while(k <= _local_7)
                {
                    ...
                    {
                        if(param1.blockWalking) blockedRow[k] = true;
                    }
                    k++;
                }
                _local_2++;
        ...
        console.log('[RoomPlaneParser] initializeHole COMPLETE - id:', param1.x, param1.y, 'invis:', param1.invisibility, 'walk:', param1.blockWalking, 'alphaHide:', param1.alphaHide);
    }

    public isFurnitureHidden(x: number, y: number, width: number, height: number): boolean
    {
        let isHidden = false;
        let hasInvertedHide = false;
        let insideInverted = false;

        if(this._floorHolesInverted && this._floorHolesInverted.size > 0)
        {
            for(const hole of this._floorHolesInverted.values())
            {
                if(hole.alphaHide === true)
                {
                    hasInvertedHide = true;
                    if (x >= hole.x && x < hole.x + hole.width && y >= hole.y && y < hole.y + hole.height) insideInverted = true;
                }
            }
        }

        if (hasInvertedHide && !insideInverted) isHidden = true;

        if(!isHidden && this._floorHoles && this._floorHoles.size > 0)
        {
            for(const hole of this._floorHoles.values())
            {
                if(hole.alphaHide === true)
                {
                    if (x >= hole.x && x < hole.x + hole.width && y >= hole.y && y < hole.y + hole.height)
                    {
                        isHidden = true;
                        break;
                    }
        ...
        if (isHidden) console.log(`[RoomPlaneParser] hiding furni at (${x}, ${y}) because hideFurniture is checked`);
        return isHidden;
    }

    public isWallItemHidden(x: number, y: number): boolean
    {
        const checkTiles = [
        ...
        if(this._floorHolesInverted && this._floorHolesInverted.size > 0)
        {
            for(const hole of this._floorHolesInverted.values())
            {
                ...
                {
                    hasInvertedHide = true;
                    const xMax = hole.x + hole.width;
                    const yMax = hole.y + hole.height;

                    for(const tile of checkTiles)
                    {
                        if (tile.x >= hole.x && tile.x < xMax && tile.y >= hole.y && tile.y < yMax) {
                            anyInSafeArea = true;
                            break;
                        }
                    }
                }
                if (anyInSafeArea) break;
            }
        }
        
        if (hasInvertedHide && !anyInSafeArea) return true;

        if(this._floorHoles && this._floorHoles.size > 0)
        {
            for(const hole of this._floorHoles.values())
            {
                if(hole.alphaHide === true)
    ...
    private extractPlanes(param1: number[][], param2: number = 0, param3: number = 0, param4: number = -1, param5: number = -1, param6: boolean = false): void
    {
        ...
        while(_loc24_ < _loc27_)
        {
            ...
            {
                if(((_loc9_ = param1[_loc24_][_loc25_]) < 0 && (!param6 || _loc9_ !== RoomPlaneParser.TILE_HOLE)))
                {
                    // 
                }
                else
                {
                   if(param6 && _loc9_ === RoomPlaneParser.TILE_HOLE)
                   {
                        _loc9_ = (this.floorHeight * 4);
                   }
                   
                   if(!_loc7_[_loc24_][_loc25_])
                   {
                    let refVal = (_loc25_ > 0) ? param1[_loc24_][_loc25_ - 1] : -999999;
                    if(param6 && refVal === RoomPlaneParser.TILE_HOLE) refVal = (this.floorHeight * 4);
                    _loc10_ = _loc25_ == 0 || refVal != _loc9_;

                    let refValTop = (_loc24_ > 0) ? param1[_loc24_ - 1][_loc25_] : -999999;
                    if(param6 && refValTop === RoomPlaneParser.TILE_HOLE) refValTop = (this.floorHeight * 4);
                    _loc8_ = _loc24_ == 0 || refValTop != _loc9_;

                    _loc19_ = _loc25_ + 1;
                    while(_loc19_ < _loc22_)
                    {
                        let nextVal = param1[_loc24_][_loc19_];
                        if(param6 && nextVal === RoomPlaneParser.TILE_HOLE) nextVal = (this.floorHeight * 4);

                        let topVal = (_loc24_ > 0) ? param1[_loc24_ - 1][_loc19_] : -999999;
                        if(param6 && topVal === RoomPlaneParser.TILE_HOLE) topVal = (this.floorHeight * 4);

                        if(nextVal != _loc9_ || _loc7_[_loc24_][_loc19_] || (_loc24_ > 0 && (topVal == _loc9_) == _loc8_))
                        {
                            break;
                        }
                        _loc19_++;
                    ...
                    let resultVal = param1[_loc24_][_loc19_];
                    if(param6 && resultVal === RoomPlaneParser.TILE_HOLE) resultVal = (this.floorHeight * 4);
                    _loc20_ = _loc19_ == _loc26_ || resultVal != _loc9_;

                    _loc11_ = false;
                    _loc16_ = _loc24_ + 1;
                    while(_loc16_ <= _loc27_ && !_loc11_)
                    {
                        let checkVal = (_loc16_ < _loc14_) ? param1[_loc16_][_loc25_] : -999999;
                        if(param6 && checkVal === RoomPlaneParser.TILE_HOLE) checkVal = (this.floorHeight * 4);

                        _loc12_ = _loc16_ == _loc14_ || checkVal != _loc9_;
                        
                        let leftSideVal = (_loc25_ > 0 && _loc16_ < _loc14_) ? param1[_loc16_][_loc25_ - 1] : -999999;
                        if(param6 && leftSideVal === RoomPlaneParser.TILE_HOLE) leftSideVal = (this.floorHeight * 4);

                        let rightSideVal = (_loc19_ < _loc26_ && _loc16_ < _loc14_) ? param1[_loc16_][_loc19_] : -999999;
                        if(param6 && rightSideVal === RoomPlaneParser.TILE_HOLE) rightSideVal = (this.floorHeight * 4);

                        _loc11_ = _loc16_ == _loc27_ || _loc12_ || (_loc25_ > 0 && (leftSideVal == _loc9_) == _loc10_ || _loc19_ < _loc26_ && (rightSideVal == _loc9_) == _loc20_);
                        if(_loc16_ == _loc14_)
                        {
                            break;
                        }
                        ...
                        {
                            let rowVal = param1[_loc16_][_loc21_];
                            if(param6 && rowVal === RoomPlaneParser.TILE_HOLE) rowVal = (this.floorHeight * 4);

                            if((rowVal == _loc9_) == _loc12_)
                            {
                                _loc11_ = true;
                                _loc19_ = _loc21_;
                                break;
