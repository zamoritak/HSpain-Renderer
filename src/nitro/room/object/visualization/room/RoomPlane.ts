...
export class RoomPlane implements IRoomPlane
{
    ...
    private _customColor: number = -1;
    private _customLineAlpha: number = 20;
    ...
    public set customColor(k: number)
    {
        if(this._customColor === k) return;
    ...
    public get customLineAlpha(): number
    {
        return this._customLineAlpha;
    }

    public set customLineAlpha(alpha: number)
    {
        if(this._customLineAlpha === alpha) return;

        this._customLineAlpha = alpha;
            ...
            if(!this._bitmapData || (this._width !== this._bitmapData.width) || (this._height !== this._bitmapData.height))
            {
                if(this._bitmapData)
                {
                    if((this._width !== this._bitmapData.width) || (this._height !== this._bitmapData.height))
                    {
                        ...
                        this._bitmapData.resize(this._width, this._height);
                    }
    ...
    private draw(k: RenderTexture, matrix: Matrix): void
    {
        ...
        if (this._customColor > -1)
        {
            if(!this._graphics) this._graphics = new Graphics();

            // 1. Draw solid base with seam-fix stroke (Screen Space)
            this._graphics.clear();
            this._graphics.lineStyle(1, this._customColor, 1);
            this._graphics.beginFill(this._customColor);

            this._graphics.moveTo(Math.round(this._cornerA.x), Math.round(this._cornerA.y));
            this._graphics.lineTo(Math.round(this._cornerB.x), Math.round(this._cornerB.y));
            this._graphics.lineTo(Math.round(this._cornerC.x), Math.round(this._cornerC.y));
            this._graphics.lineTo(Math.round(this._cornerD.x), Math.round(this._cornerD.y));
            this._graphics.lineTo(Math.round(this._cornerA.x), Math.round(this._cornerA.y));

            this._graphics.endFill();
            
            this._textureCache.writeToRenderTexture(this._graphics, this._bitmapData, true, null);

            // 2. MASKING: Use the original texture 'k' (which has the door hole) to cut the custom color
            // DST_IN keeps the destination (custom color) only where the source ('k') is opaque.
            const maskSprite = new Sprite(k);
            maskSprite.blendMode = BLEND_MODES.DST_IN;
            this._textureCache.writeToRenderTexture(maskSprite, this._bitmapData, false, matrix);

            // 3. Draw Floor Grid Overlay (On top of masked color)
            if(this._type === RoomPlane.TYPE_FLOOR)
            {
                this._graphics.clear();
                
                const r = (this._customColor >> 16) & 0xFF;
                const g = (this._customColor >> 8) & 0xFF;
                const b = this._customColor & 0xFF;
                const luminance = (0.299 * r + 0.587 * g + 0.114 * b);
                const lineColor = (luminance < 128) ? 0xFFFFFF : 0x000000;

                const dimX = Math.round(this._leftSide.length);
                const dimY = Math.round(this._rightSide.length);
                
                this._graphics.lineStyle(1, lineColor, (this._customLineAlpha / 100));

                for (let i = 1; i < dimX; i++)
                {
                    const r = i / dimX;
                    const p1x = this._cornerA.x + (this._cornerD.x - this._cornerA.x) * r;
                    const p1y = this._cornerA.y + (this._cornerD.y - this._cornerA.y) * r;
                    const p2x = this._cornerB.x + (this._cornerC.x - this._cornerB.x) * r;
                    const p2y = this._cornerB.y + (this._cornerC.y - this._cornerB.y) * r;
                    
                    this._graphics.moveTo(p1x, p1y);
                    this._graphics.lineTo(p2x, p2y);
                }

                for (let i = 1; i < dimY; i++)
                {
                    const r = i / dimY;
                    const p1x = this._cornerA.x + (this._cornerB.x - this._cornerA.x) * r;
                    const p1y = this._cornerA.y + (this._cornerB.y - this._cornerA.y) * r;
                    const p2x = this._cornerD.x + (this._cornerC.x - this._cornerD.x) * r;
                    const p2y = this._cornerD.y + (this._cornerC.y - this._cornerD.y) * r;
                    
                    this._graphics.moveTo(p1x, p1y);
                    this._graphics.lineTo(p2x, p2y);
                }

                this._textureCache.writeToRenderTexture(this._graphics, this._bitmapData, false, null);
            }
        }
        else
        {
        ...
        if(this.isHighlighter)
        {
            ...
            this._textureCache.writeToRenderTexture(this._graphics, this._bitmapData, false);
        }
    }

    public get cornerA(): Vector3d
    {
        return this._cornerA;
    }

    public get cornerB(): Vector3d
    {
        return this._cornerB;
    }

    public get cornerC(): Vector3d
    {
        return this._cornerC;
    }

    public get cornerD(): Vector3d
    {
        return this._cornerD;
    }
    ...
    private updateMask(canvas: RenderTexture, geometry: IRoomGeometry): void
    {
        ...
        {
            this._maskBitmapData = this._textureCache.createRenderTexture(width, height, 'mask');
            this._maskChanged = true;
        }
        ...
        if(this._maskChanged)
        {
            ...
            if(this._maskBitmapData)
            {
               PixiApplicationProxy.instance.renderer.render(new Sprite(Texture.EMPTY), {
                    renderTexture: this._maskBitmapData,
                    clear: true
                });
            }
            ...
            while(i < this._rectangleMasks.length)
            {
                ...
                if(rectMask)
                {
                    ...
                    graphics.drawRect(0, 0, wd, ht);
                    graphics.endFill();
                    
                    graphics.position.set((posX - wd), (posY - ht));

                    this._textureCache.writeToRenderTexture(graphics, this._maskBitmapData, false);
        ...
        this.combineTextureMask(canvas);
    }

    private combineTextureMask(canvas: RenderTexture): void
    {
        if(!canvas || !this._maskBitmapData) return;

        const sprite = new Sprite(this._maskBitmapData);

        sprite.blendMode = BLEND_MODES.DST_OUT;

        this._textureCache.writeToRenderTexture(sprite, canvas, false);
    }
}
