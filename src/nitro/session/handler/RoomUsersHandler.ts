...
export class RoomUsersHandler extends BaseHandler
{
    ...
    private onRoomUnitEvent(event: RoomUnitEvent): void
    {
        ...
        if(users && users.length)
        {
            for(const user of users)
            {
                ...
                userData.usernameIcon = user.usernameIcon;
                userData.usernameEffect = user.usernameEffect;
                userData.ornament = user.ornament;
                userData.textColor = user.textColor;
    ...
    private onRoomUnitInfoEvent(event: RoomUnitInfoEvent): void
    {
		...
		session.userDataManager.updateAppearance(
        parser.unitId,
        parser.backgroundId,    // background
        parser.standId,         // stand
        parser.overlayId,       // overlay
        parser.usernameIcon,    // usernameIcon
        parser.usernameEffect,  // usernameEffect
        parser.ornament,       // ornament
        parser.textColor       // textColor
    );

    // Después de actualizar la apariencia, lanzamos el evento con los nuevos datos.
    this.listener.events.dispatchEvent(new RoomSessionUserFigureUpdateEvent(
        session,
        parser.unitId,
        parser.figure,
        parser.gender,
